            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form action="{{ route('users.update', $user->id) }}" method="post">
                        ...
                        <div class="my-3">
                            <input value="{{ old('name', $user->name) }}" name="name" placeholder="Enter Roles Name"
                        ...
                        <div class="grid grid-cols-4 mb-4">
                            @if ($roles->isNotEmpty())
                                @foreach ($roles as $role)
                                    <div class="mt-3">
                                        {{--  --}}
                                        <input {{ ($hasRoles->contains($role->id)) ? 'checked' : '' }} id="role-{{ $role->id }}" type="checkbox" class="rounded" name="role[]" value="{{ $role->name }}" id="">
